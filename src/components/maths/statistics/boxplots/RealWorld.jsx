...
const SCENARIOS = [
  {
    ...
    whySortExplain: "We sort the data so we can find the middle value (median) and the quartiles. If the numbers are jumbled, it is impossible to tell which value is in the middle. Sorting is always the first step.",
    contextExplain: {
      ...
      median: "Median = 45 minutes is the typical waiting time - half of patients waited less, half waited more. This is the number a hospital manager cares most about.",
  ...
  },
  {
    ...
    setup: "30 identical seedlings were treated with Fertiliser A (standard) and 30 with Fertiliser B (new). After 4 weeks, the heights (in cm) were recorded. The results are summarised in a frequency table and bar chart below.",
    unit: "cm",
    unitShort: "cm",
    ...
    working: [
      ...
      { label: "Q1 - position: 1/4 x 30 = 7.5, round up to 8th value", value: "Falls in 11-15 interval - Q1 = 14 cm" },
      { label: "Median - position: 1/2 x 30 = 15th value", value: "Falls in 16-20 interval - Median = 19 cm" },
      { label: "Q3 - position: 3/4 x 30 = 22.5, round up to 23rd value", value: "Falls in 21-25 interval - Q3 = 25 cm" },
    ...
    contextExplain: {
      ...
      median: "Median = 19 cm is the typical height - half the plants grew taller, half shorter. This is the headline number for comparing fertilisers.",
    ...
    decisionText: "A scientist comparing fertilisers A and B would look at: which has the higher median (better average growth)? Which has the smaller IQR (more reliable, consistent results)? A fertiliser that only works on some plants is not as useful as one that works consistently on all of them.",
  ...
  },
  {
    ...
    setup: "A courier company tracked how many days 60 parcels took to arrive after dispatch. The results are shown as a cumulative frequency graph. Use it to find Q1, the median and Q3, then draw the box plot.",
    unit: "days",
    unitShort: "days",
    answer: { min: 1, q1: 3, median: 5, q3: 7, max: 12 },
    totalFreq: 60,
    cumFreqPoints: [
      [0,0],[1,0],[2,3],[3,15],[4,24],[5,30],[6,38],[7,45],[8,51],[9,55],[10,57],[11,59],[12,60],
    ],
    readPoints: [
    ...
    contextExplain: {
      ...
      median: "Median = 5 days is the typical delivery time. Half of parcels took less than 5 days, half took more. If you promise '5 working days', you would be right about half the time.",
    ...
    decisionText: "The business owner might switch to a more reliable courier, or offer tracking so customers know what to expect. The IQR of 4 days is the real problem - it is unpredictable. Customers can tolerate slow if it is consistent; they hate not knowing when to expect their parcel.",
    scaleMin: 0, scaleMax: 14,
  },
];
  ...
  return (
    <svg viewBox={`0 0 ${W} ${padT + plotH + padB}`} style={{ width: "100%", overflow: "visible" }}>
      ...
      {rows.map((row, i) => {
        ...
        return (
          <g key={row.interval}>
            {h > 0 && <rect x={x + 2} y={y} width={barW - 4} height={h} fill={barColor} opacity="0.75" rx="3" />}
            {row.freq > 0 && <text x={x + barW / 2} y={y - 4} textAnchor="middle" fill={barColor} fontSize="9" fontWeight="700">{row.freq}</text>}
            <text x={x + barW / 2} y={padT + plotH + 14} textAnchor="middle" fill={C.muted} fontSize="8"
              transform={`rotate(-35, ${x + barW / 2}, ${padT + plotH + 14})`}>{row.interval}</text>
          </g>
        );
      })}
      ...
      <text x={padL + plotW / 2} y={padT + plotH + padB - 4} textAnchor="middle" fill={C.muted} fontSize="10">{xLabel || "Height (cm)"}</text>
    </svg>
  );
}
...
function CumFreqSVG({ points, totalFreq, readPoints, unit, scaleMax }) {
  ...
  const pathD = points.map(([v, f], i) => `${i === 0 ? "M" : "L"} ${toX(v)} ${toY(f)}`).join(" ");
  ...
  return (
    <svg viewBox={`0 0 ${W} ${padT + plotH + padB}`} style={{ width: "100%", overflow: "visible" }}>
      ...
      {readPoints.map(({ freq, value, color }) => (
        <g key={freq}>
          <line x1={padL} y1={toY(freq)} x2={toX(value)} y2={toY(freq)} stroke={color} strokeWidth="1.5" strokeDasharray="5,3" opacity="0.8" />
          <line x1={toX(value)} y1={toY(freq)} x2={toX(value)} y2={toY(0)} stroke={color} strokeWidth="1.5" strokeDasharray="5,3" opacity="0.8" />
          ...
          <rect x={toX(value) - 8} y={toY(0) + 20} width="16" height="13" fill="#fff" />
          <text x={toX(value)} y={toY(0) + 31} textAnchor="middle" fill={color} fontSize="10" fontWeight="700">{value}</text>
        </g>
      ))}
...
function Hint({ title, children, color }) {
  ...
  return (
    <div style={{ border: `1px solid ${border}`, borderRadius: "10px", marginBottom: "10px", overflow: "hidden" }}>
      ...
      {open && <div style={{ background: bg, padding: "12px 14px", borderTop: `1px solid ${border}` }}>{children}</div>}
    </div>
  );
}

// ── RAW DATA STEPS (hospital) ─────────────────────────────────────────────
function Step1Raw({ s, onNext }) {
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "20px" }}>
        {s.rawData.map((v, i) => (
          <div key={i} style={{ minWidth: "44px", height: "44px", borderRadius: "10px", background: C.surface,
            ...
            fontSize: "14px", fontWeight: "600", color: C.text }}>{v}</div>
        ))}
      </div>
      <p style={{ fontSize: "13px", color: C.muted, lineHeight: 1.6, marginBottom: "20px" }}>
        These numbers are in the order patients arrived - not in any useful order. Before we can find the median or quartiles, we need to sort them.
      </p>
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "16px" }}>
        {s.rawData.map((v, i) => (
          <div key={i} style={{ minWidth: "40px", height: "40px", borderRadius: "8px", background: C.surface,
            border: `1.5px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: "13px", color: C.muted }}>{v}</div>
        ))}
      </div>
      <p style={{ fontSize: "13px", fontWeight: "700", color: C.text, marginBottom: "6px" }}>Sorted (lowest to highest):</p>
        ...
        {s.sorted.map((v, i) => {
          ...
          return (
            <div key={i} style={{ minWidth: "40px", height: "40px", borderRadius: "8px",
              ...
              color: isMedian ? "#fff" : isQ1 || isQ3 ? C.accent : C.text, position: "relative" }}>
              {v}
      ...
      <div style={{ background: C.accentDim, border: `1px solid ${C.accent}40`, borderRadius: "10px", padding: "12px 14px", marginBottom: "20px" }}>
        <p style={{ fontSize: "13px", color: C.text, margin: 0, lineHeight: 1.7 }}>
          With {s.sorted.length} values (odd), the median is the <strong style={{ color: C.accent }}>{Math.floor(s.sorted.length / 2) + 1}th value = {s.answer.median}</strong>. Q1 is the median of the lower half. Q3 is the median of the upper half.
        </p>
      </div>
      <button onClick={onNext} style={{ width: "100%", padding: "14px", background: C.accent, color: "#fff",
        border: "none", borderRadius: "10px", fontSize: "14px", fontWeight: "700", cursor: "pointer" }}>
...
function Step1Bar({ s, onNext }) {
  const [activeTab, setActiveTab] = useState("A");
  const rows = activeTab === "A" ? s.rows : s.rowsB;
  const fertiliserColor = activeTab === "A" ? C.accent : "#d97706";
  const fertiliserLabel = activeTab === "A" ? "Fertiliser A (standard)" : "Fertiliser B (new)";
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "6px", marginBottom: "14px", background: C.surface, borderRadius: "10px", padding: "4px", border: `1px solid ${C.border}` }}>
        {["A", "B"].map(f => (
          <button key={f} onClick={() => setActiveTab(f)}
            style={{ flex: 1, padding: "8px", borderRadius: "8px", border: "none", cursor: "pointer",
              background: activeTab === f ? (f === "A" ? C.accent : "#d97706") : "transparent",
      ...
      {/* Bar chart */}
      <div style={{ background: C.surface, border: `1px solid ${fertiliserColor}40`, borderRadius: "12px", padding: "16px", marginBottom: "14px" }}>
        <p style={{ fontSize: "12px", fontWeight: "700", color: fertiliserColor, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "10px" }}>
      ...
      {/* Frequency table */}
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "14px", marginBottom: "14px" }}>
        <p style={{ fontSize: "12px", fontWeight: "700", color: C.text, marginBottom: "10px" }}>Frequency table - {fertiliserLabel}</p>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
          <thead>
            <tr style={{ borderBottom: `2px solid ${C.border}` }}>
              {["Height (cm)", "Frequency", "Cumulative frequency"].map(h => (
          ...
          <tbody>
            {rows.map((row, i) => {
              ...
              const isKey = keyPositions.some(pos => pos > prevCum && pos <= row.cumFreq);
              return (
                <tr key={row.interval} style={{ background: isKey ? fertiliserColor + "18" : "transparent", borderBottom: `1px solid ${C.border}` }}>
                  <td style={{ padding: "6px 8px", color: isKey ? fertiliserColor : C.text, fontWeight: isKey ? "700" : "400" }}>{row.interval}</td>
                  <td style={{ padding: "6px 8px", textAlign: "center", color: isKey ? fertiliserColor : C.text, fontWeight: isKey ? "700" : "400" }}>{row.freq}</td>
                  <td style={{ padding: "6px 8px", textAlign: "center", color: isKey ? fertiliserColor : C.muted, fontWeight: isKey ? "700" : "400" }}>{row.cumFreq}</td>
                </tr>
              );
            })}
          </tbody>
        </table>
        <p style={{ fontSize: "11px", color: fertiliserColor, margin: "8px 0 0" }}>Highlighted rows contain Q1, Median or Q3</p>
      </div>

      {/* Key positions helper */}
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "10px", padding: "12px 14px", marginBottom: "16px" }}>
        <p style={{ fontSize: "12px", fontWeight: "700", color: C.text, margin: "0 0 8px" }}>Key positions to find (30 plants total)</p>
        {[
      ...
      <div style={{ background: "#fffbeb", border: "1px solid #d97706", borderRadius: "8px", padding: "10px 12px", marginBottom: "20px" }}>
        <p style={{ fontSize: "12px", color: "#92400e", margin: 0, lineHeight: 1.6 }}>
          💡 Switch between Fertiliser A and B above to compare their distributions. Notice how B is shifted to the right - more plants grew taller. In the next step you will find the five-number summary for Fertiliser A, then compare.
        </p>
      </div>

      <button onClick={onNext} style={{ width: "100%", padding: "14px", background: C.accent, color: "#fff",
        border: "none", borderRadius: "10px", fontSize: "14px", fontWeight: "700", cursor: "pointer" }}>
        Next: Find Q1, Median, Q3 for Fertiliser A →
      </button>
    </div>
  );
}
...
function Step1CumFreq({ s, onNext }) {
  return (
    <div>
      <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: "10px", padding: "14px", marginBottom: "16px" }}>
        <p style={{ fontSize: "13px", fontWeight: "700", color: "#15803d", margin: "0 0 6px" }}>{s.icon} Your task</p>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "16px", marginBottom: "14px" }}>
        <p style={{ fontSize: "12px", fontWeight: "700", color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "4px" }}>Cumulative frequency graph - 60 parcels</p>
      ...
      <div style={{ background: C.accentDim, border: `1px solid ${C.accent}40`, borderRadius: "10px", padding: "12px 14px", marginBottom: "16px" }}>
        <p style={{ fontSize: "13px", fontWeight: "700", color: C.accent, margin: "0 0 8px" }}>How to read this graph</p>
        {[
          { n: "1", text: "Find the frequency on the y-axis: Q1 = 15, Median = 30, Q3 = 45" },
          { n: "2", text: "Draw a horizontal line across to the curve" },
          { n: "3", text: "Drop straight down to the x-axis - that is your value" },
        ].map(({ n, text }) => (
          <div key={n} style={{ display: "flex", gap: "10px", marginBottom: "6px" }}>
            <div style={{ width: "20px", height: "20px", borderRadius: "50%", background: C.accent, color: "#fff",
              display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: "700", flexShrink: 0 }}>{n}</div>
            <p style={{ fontSize: "12px", color: C.text, margin: 0, lineHeight: 1.6 }}>{text}</p>
          </div>
        ))}
      </div>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "10px", padding: "12px 14px", marginBottom: "20px" }}>
        <p style={{ fontSize: "12px", fontWeight: "700", color: C.text, margin: "0 0 8px" }}>Values read from the graph</p>
        {[
          { label: "Min", value: `${s.answer.min} ${s.unitShort}`, note: "Given in question - fastest delivery" },
          { label: "Q1", value: `${s.answer.q1} ${s.unitShort}`, note: "Read from graph at frequency 15" },
          { label: "Median", value: `${s.answer.median} ${s.unitShort}`, note: "Read from graph at frequency 30" },
          { label: "Q3", value: `${s.answer.q3} ${s.unitShort}`, note: "Read from graph at frequency 45" },
          { label: "Max", value: `${s.answer.max} ${s.unitShort}`, note: "Given in question - slowest delivery" },
          ...
          <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0", borderBottom: `1px solid ${C.border}` }}>
            <div>
              <span style={{ fontSize: "13px", fontWeight: "700", color: C.accent }}>{label}</span>
              <span style={{ fontSize: "11px", color: C.muted, marginLeft: "8px" }}>{note}</span>
            </div>
...
function Step3({ s, onNext }) {
  ...
  const items = [
    { key: "min",    label: "Minimum",          value: min,    hint: s.type === "barchart" ? "The smallest value in the first interval of the frequency table." : s.type === "cumfreq" ? "Given directly in the question - the fastest delivery." : "The first (smallest) number in the sorted list." },
    { key: "q1",     label: "Q1 - Lower Quarter", value: q1,   hint: s.working ? s.working[2].value : s.type === 'cumfreq' ? `Read from the graph at frequency ${s.totalFreq / 4}. The curve crosses at ${q1} ${s.unitShort}.` : `Take the bottom half of the data (below the median). Find the middle of those values. Here that is ${q1} ${s.unitShort}.` },
    { key: "median", label: "Median",            value: median, hint: s.working ? s.working[3].value : s.type === 'cumfreq' ? `Read from the graph at the halfway point (frequency ${s.totalFreq / 2}). The curve crosses at ${median} ${s.unitShort}.` : `${s.sorted.length} values - odd number. The middle one is position ${Math.floor(s.sorted.length / 2) + 1} = ${median} ${s.unitShort}.` },
    { key: "q3",     label: "Q3 - Upper Quarter", value: q3,   hint: s.working ? s.working[4].value : s.type === 'cumfreq' ? `Read from the graph at frequency ${s.totalFreq * 3 / 4}. The curve crosses at ${q3} ${s.unitShort}.` : `Take the upper half of the data (above the median). Find the middle of those values. Here that is ${q3} ${s.unitShort}.` },
    { key: "max",    label: "Maximum",           value: max,    hint: s.type === "barchart" ? "The largest value in the last interval of the frequency table." : s.type === "cumfreq" ? "Given directly in the question - the slowest delivery." : "The last (largest) number in the sorted list." },
    { key: "iqr",    label: "IQR",              value: iqr,    hint: `IQR = Q3 - Q1 = ${q3} - ${q1} = ${iqr} ${s.unitShort}` },
  ];

  return (
    <div>
      <Hint title={s.type === "barchart" ? "📊 What do these numbers mean - Fertiliser A?" : "📊 What do these numbers actually mean in this context?"} color="amber">
      ...
      {/* For barchart type, make clear which fertiliser these values belong to */}
      {s.type === "barchart" ? (
        <div style={{ marginTop: "16px", marginBottom: "10px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
            <div style={{ width: "12px", height: "12px", borderRadius: "50%", background: C.accent, flexShrink: 0 }} />
            <p style={{ fontSize: "13px", fontWeight: "700", color: C.text, margin: 0 }}>
              5-number summary - <span style={{ color: C.accent }}>Fertiliser A (standard)</span>
            </p>
          </div>
          <p style={{ fontSize: "12px", color: C.muted, margin: "0 0 10px", lineHeight: 1.5 }}>
            These values come from Fertiliser A's frequency table. Fertiliser B's five-number summary will appear in the comparison on the next step.
          </p>
        </div>
      ) : (
...
function Step4({ s, onRestart }) {
  ...
  const sets = isScience
    ? [{ ...s.answer, label: "Fertiliser A (standard)", color: C.accent }, { ...s.answerB, label: "Fertiliser B (new)", color: "#d97706" }]
    : [{ ...s.answer, label: s.title, color: C.accent }];
  return (
    <div>
      <p style={{ fontSize: "13px", color: C.muted, marginBottom: "14px", lineHeight: 1.6 }}>
        All 5 values plotted on a single diagram. This is what a manager, scientist, or analyst would actually look at.
      </p>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "16px", marginBottom: "16px" }}>
        {isScience && (
          <p style={{ fontSize: "11px", fontWeight: "700", color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "12px" }}>
      ...
      {isScience && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "14px", marginBottom: "16px" }}>
          <p style={{ fontSize: "12px", fontWeight: "700", color: C.text, margin: "0 0 10px" }}>Side-by-side comparison</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
            {[{ label: "Fertiliser A", data: s.answer, color: C.accent }, { label: "Fertiliser B", data: s.answerB, color: "#d97706" }].map(({ label, data, color }) => (
              <div key={label} style={{ background: color + "10", border: `1.5px solid ${color}40`, borderRadius: "10px", padding: "10px 12px" }}>
                <p style={{ fontSize: "11px", fontWeight: "700", color, margin: "0 0 6px" }}>{label}</p>
                {[["Median", data.median], ["IQR", data.q3 - data.q1], ["Min", data.min], ["Max", data.max]].map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: `1px solid ${C.border}` }}>
                    ...
                    <span style={{ fontSize: "12px", fontWeight: "700", color: k === "Median" || k === "IQR" ? color : C.text }}>{v} cm</span>
                  </div>
                ))}
              </div>
            ))}
          </div>
          <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: "8px", padding: "10px 12px", marginTop: "10px" }}>
            <p style={{ fontSize: "12px", color: "#15803d", margin: 0, lineHeight: 1.6 }}>
              <strong>Conclusion:</strong> Fertiliser B has a higher median ({s.answerB.median} cm vs {s.answer.median} cm) so it grows taller plants on average. Its IQR is {s.answerB.q3 - s.answerB.q1} cm vs {s.answer.q3 - s.answer.q1} cm for A - B is slightly less consistent, but the higher average growth makes it the better choice.
            </p>
          </div>
        </div>
      )}
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "14px", marginBottom: "20px" }}>
        ...
        {[
          { label: "Median", value: s.answer.median, unit: s.unitShort, desc: "Typical result", term: "Median" },
          { label: "IQR", value: s.answer.q3 - s.answer.q1, unit: s.unitShort, desc: "Consistency (smaller = better)", term: "IQR" },
          { label: "Range", value: s.answer.max - s.answer.min, unit: s.unitShort, desc: "Total spread", term: "Range" },
          { label: "Min", value: s.answer.min, unit: s.unitShort, desc: "Best case", term: "Minimum" },
          { label: "Max", value: s.answer.max, unit: s.unitShort, desc: "Worst case", term: "Maximum" },
      ...
      <div style={{ background: "#fffbeb", border: "1px solid #d97706", borderRadius: "10px", padding: "12px 14px", marginBottom: "20px" }}>
        <p style={{ fontSize: "12px", color: "#92400e", margin: 0, lineHeight: 1.7 }}>
          ⭐ <strong>Exam answer:</strong> "{s.title} had a median of {s.answer.median} {s.unitShort} and an IQR of {s.answer.q3 - s.answer.q1} {s.unitShort}."
...
export default function RealWorld() {
    ...
    return (
      <div>
        <p style={{ fontSize: "13px", color: C.muted, marginBottom: "16px", lineHeight: 1.6 }}>
          Choose a real-world situation. Each one shows a different way data can be presented - raw numbers, a bar chart, or a cumulative frequency graph.
        </p>
          ...
          {SCENARIOS.map(s => (
            <button key={s.id} onClick={() => { setSelected(s.id); setStep(0); scrollToTop(); }}
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: "12px", color: C.muted, margin: "0 0 4px" }}>{s.hook}</p>
                <span style={{ fontSize: "10px", fontWeight: "700", padding: "2px 8px", borderRadius: "99px",
                  background: s.type === "raw" ? C.accentDim : s.type === "barchart" ? "#fffbeb" : "#f5f3ff",
                  color: s.type === "raw" ? C.accent : s.type === "barchart" ? "#d97706" : "#7c3aed",
                  border: `1px solid ${s.type === "raw" ? C.accent + "40" : s.type === "barchart" ? "#fde68a" : "#ddd6fe"}` }}>
                  {s.type === "raw" ? "Raw data" : s.type === "barchart" ? "Bar chart" : "Cumulative frequency graph"}
                </span>
              </div>
  ...
  return (
    <div>
      ...
      {isRaw && step === 0 && <Step1Raw s={s} onNext={() => { setStep(1); scrollToTop(); }} />}
      {isRaw && step === 1 && <Step2Raw s={s} onNext={() => { setStep(2); scrollToTop(); }} />}
      {isRaw && step === 2 && <Step3 s={s} onNext={() => { setStep(3); scrollToTop(); }} />}
      {isRaw && step === 3 && <Step4 s={s} onRestart={() => { setSelected(null); setStep(0); scrollToTop(); }} />}

      {!isRaw && s.type === "barchart" && step === 0 && <Step1Bar s={s} onNext={() => { setStep(1); scrollToTop(); }} />}
      {!isRaw && s.type === "cumfreq" && step === 0 && <Step1CumFreq s={s} onNext={() => { setStep(1); scrollToTop(); }} />}
      {!isRaw && step === 1 && <Step3 s={s} onNext={() => { setStep(2); scrollToTop(); }} />}
      {!isRaw && step === 2 && <Step4 s={s} onRestart={() => { setSelected(null); setStep(0); scrollToTop(); }} />}
    </div>
  );
}