...
function StageReadGraph({ q, onComplete }) {
  ...
  const readPoints = [
    ...
    { freq: medFreq, label: `Median (position ${medFreq} of ${n})`, value: q.answer.median, color: C.white },
    ...
    return (
      <div>
        <p style={{ fontSize: "13px", color: C.muted, marginBottom: "16px", lineHeight: 1.6 }}>
          Each question has <strong style={{ color: C.text }}>4 stages</strong>: read the graph → order the data → find the 5 values → drag to build the box plot.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {BUILD_QUESTIONS.map((bq, i) => (
            <button key={bq.id} onClick={() => { setQIdx(i); setStage(0); }} style={{ padding: "14px 16px", background: C.card, border: `1px solid ${C.border}`, borderRadius: "12px", cursor: "pointer", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>
        ...
        <div style={{ background: C.surface, borderRadius: "10px", padding: "12px 14px", marginTop: "16px" }}>
          <p style={{ fontSize: "12px", color: C.muted, margin: 0, lineHeight: 1.6 }}>
            💡 <strong style={{ color: C.text }}>New:</strong> Each question now starts with a cumulative frequency graph — the same type used in exam Q8 and Q9.
          </p>
        </div>
      </div>
    );
